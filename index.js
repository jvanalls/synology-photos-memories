...
const sendEmailHost = process.env.SEND_EMAIL_HOST
const sendEmailPort = process.env.SEND_EMAIL_PPORT
const sendEmailAddr = process.env.SEND_EMAIL_ADDR
const sendEmailReplyTo = process.env.SEND_EMAIL_REPLY
const receiveEmail = process.env.RECEIVE_EMAIL
const receiveEmailGmail = process.env.RECEIVE_EMAILGMAIL
const emailSubject = process.env.EMAIL_SUBJECT
const domainInternal = process.env.DOMAIN_INTERNAL
const domainExternal = process.env.DOMAIN_EXTERNAL
...
async function authenticate() {
    ...
    const authResponse = await fetch(
        `https://${ip}/webapi/auth.cgi?api=SYNO.API.Auth&version=3&method=login&account=${encodeURIComponent(user)}&passwd=${encodeURIComponent(password)}`
    )
...
function getExpectedWidth(photo) {
    const originalWidth = parseInt(photo.additional.resolution.width, 10);
    const originalHeight = parseInt(photo.additional.resolution.height, 10);
    const currentHeight = 120;
    const expectedWidth2 = Math.round((originalWidth / originalHeight) * currentHeight);
    const expectedWidth = (originalWidth / originalHeight) * currentHeight;
    const expectedWidthRt = expectedWidth + 'px';
...
function returnPhotosInfo(photos, sid) {
    return photos.map(photo => {
        ...
        return {
            ...
            expectedWidth: getExpectedWidth(photo)
        };
    });
}
...
async function main() {
    ...
    ejs.renderFile(path.join(__dirname, "views/email.ejs"), { urlList: photosRawInfo }, function (err, mailHtml) {
    ...
    ejs.renderFile(path.join(__dirname, "views/email-gmail.ejs"), { urlList: photosRawInfo }, function (err, mailHtml) {


        const mailOptions = {
            from: sendEmailAddr,
            replyTo: sendEmailReplyTo,
            to: receiveEmailGmail,
            subject: emailSubject,
            html: mailHtml
        }
